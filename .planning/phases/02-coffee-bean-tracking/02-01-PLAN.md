...
<task type="auto">
  <name>Task 2: Bean list views (BeanRow, FreshnessIndicatorView, BeanListView with search and archive)</name>
  <files>
    CoffeeJournal/Views/Beans/FreshnessIndicatorView.swift
    CoffeeJournal/Views/Beans/BeanRow.swift
    CoffeeJournal/Views/Beans/BeanListView.swift
  ...
  <action>
    Create THREE new files in a new `Views/Beans/` directory:
  ...
  <verify>
    `swift build` compiles without errors. All three view files exist in CoffeeJournal/Views/Beans/. BeanListView uses parent/child @Query pattern with #Predicate. BeanRow renders freshness indicator. BeanListView has search and archive toggle.
  </verify>
  <done>
    Bean list UI complete: FreshnessIndicatorView shows monochrome freshness badge, BeanRow shows bean summary with photo and freshness, BeanListView supports search-by-roaster-or-origin and active/archived segmented filter with swipe-to-archive and swipe-to-delete.
  </done>
</task>

<task type="auto">
  <name>Task 3: Bean CRUD forms (AddBeanView and BeanDetailView with photo picker)</name>
  <files>
    CoffeeJournal/Views/Beans/AddBeanView.swift
    CoffeeJournal/Views/Beans/BeanDetailView.swift
  </files>
  <action>
    Create TWO new files in the `Views/Beans/` directory:
  ...
  <verify>
    `swift build` compiles without errors. Both files exist in CoffeeJournal/Views/Beans/. AddBeanView has Cancel/Save toolbar with validation. BeanDetailView uses @Bindable pattern with photo picker and freshness indicator.
  </verify>
  <done>
    Bean CRUD forms complete: AddBeanView provides full entry form with all BEAN-01 fields (roaster, origin, region, variety, processing method, roast level, roast date, notes) with validation. BeanDetailView provides @Bindable edit view with photo picker, freshness display, and archive control.
  </done>
</task>

<task type="auto">
  <name>Task 4: Wire Beans tab into MainTabView</name>
  <files>
    CoffeeJournal/Views/MainTabView.swift
  </files>
  <action>
