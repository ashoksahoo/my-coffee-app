...
must_haves:
  ...
  artifacts:
    ...
    - path: "CoffeeJournal/Views/Brewing/AddBrewLogView.swift"
      provides: "Modal form for brew log creation with equipment/bean pickers, parameters, rating, notes"
      min_lines: 100
...
<objective>
Create the brew log entry form with equipment/bean selection, core brewing parameters, auto-calculated ratio, method-specific parameters, star rating, and freeform notes.
...
- Set brewTime from either manual entry or timer elapsed seconds (whichever is nonzero)
- context.insert(log)
...
Run `swift build` from the project root. BrewLog.swift compiles with grinderSetting field. BrewLogViewModel.swift compiles with all properties, computed values, and saveBrew function. No errors.
  ...
  <done>
BrewLog model has grinderSetting field and computed ratio/time formatting. BrewLogViewModel manages full form state with equipment selection, parameter entry, ratio calculation, grinder clamping, validation, and save logic including equipment stat updates.
  </done>
</task>
...
**Section: "Brew Time" (manual entry for now, Plan 02 adds timer)** -- TextField for manual time entry. Accept minutes and seconds as a simple text field or two separate Steppers (minutes Stepper 0...60, seconds Stepper 0...59). Store as total seconds in viewModel.brewTime.
...
Note: The @Query properties for methods, grinders, and beans should be declared at the top of AddBrewLogView (or in small child section views that hold the @Query). If using child views per picker section, pass `@Bindable var viewModel: BrewLogViewModel` to them.
  ...
  <verify>
Run `swift build` from the project root. Both files compile without errors. StarRatingView exists with @Binding rating. AddBrewLogView has Form with all sections, toolbar Save/Cancel, and uses BrewLogViewModel.
  </verify>
  <done>
AddBrewLogView presents a multi-section Form for brew log creation. User can select brew method, coffee bean, grinder with constrained grind setting slider, enter dose/water/temperature/yield parameters with method-adaptive visibility, see auto-calculated brew ratio, rate 1-5 stars, write freeform notes, and save. Equipment stats update on save. Form prevents accidental dismissal when data has been entered.
  </done>
</task>
...
- BrewLogViewModel has all computed properties (brewRatio, canSave, shows* flags)
- AddBrewLogView renders Form with method/coffee/grinder pickers, parameter inputs, ratio display, rating, notes
...
The form is fully functional for creating brew logs. Timer (BREW-08, BREW-11), photos (BREW-12), list/detail views, and tab wiring are in Plans 02 and 03.
</success_criteria>
