...
must_haves:
  truths:
    ...
    - "Step guidance advances automatically based on timer or manually by user tap"
  ...
  key_links:
    ...
    - from: "CoffeeJournal/Views/Brewing/AddBrewLogView.swift"
      ...
      via: "Timer section embeds BrewTimerView in the form"
...
Output: BrewTimerView (embedded timer component), BrewStepGuideView (optional step overlay), BrewStepTemplates (static step data), ViewModel timer methods, AddBrewLogView updated with timer section.
</objective>
...
<task type="auto">
  <name>Task 2: Create BrewTimerView, BrewStepGuideView, and wire into AddBrewLogView</name>
...
**AddBrewLogView.swift** -- Replace the manual brew time section with the timer:

1. Replace the "Brew Time" section (manual steppers/text field from Plan 01) with a new "Brew Timer" section containing:
...
- Below the timer, show a small manual override: `Text("or enter manually")` with a TextField for manual brew time entry (minutes:seconds). This allows users to skip the timer entirely. Only show manual entry when timerState == .idle (timer not started).

2. Ensure the existing brew time manual entry still works as fallback: if timerState == .stopped, use timer's recorded time. If timerState == .idle and manual time entered, use manual time. The viewModel.saveBrew already handles this via brewTime property.

3. Do NOT add NavigationStack or modify the sheet presentation structure. Only modify the form content.
  </action>
  <verify>
Run `swift build` from the project root. All three files compile. BrewTimerView shows timer display and controls. BrewStepGuideView shows step guidance when enabled. AddBrewLogView includes timer section.
  </verify>
  <done>
Brew timer is fully integrated into the brew log form with start/pause/resume/stop controls, M:SS.t display, optional step-by-step guidance per method category (pour-over bloom/pour/drawdown, espresso pre-infusion/extraction, immersion steep), and manual time entry fallback. Timer uses Date-based calculation to prevent drift across pause/resume cycles.
  </done>
</task>
